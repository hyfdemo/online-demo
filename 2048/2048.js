...
//获取屏幕的宽高并设置对应gridPanel\.grid\.cell的元素的宽高，
//同时设置元素的位置
function setWidthHeight() {
  var width = window.innerWidth;
  var height = window.innerHeight - $('#scoreId').height();
  var cellWidth = width * (4 / 21);
  var cellHeight = height * (4 / 21);

  var base_marginLeft = width*(1/21);
  var base_marginTop = height*(1/21);

  $('#gridPanel').css({
    width:width,
    height:height,
    right:base_marginLeft/2
  })
...
//  设置cell和grid的top和left的值
  for(var r=0;r<4;r++)
  {
    for(var c=0;c<4;c++)
    {
      //console.log($('[id $= "'+r+c+'"]'));
      $('[id $= "'+r+c+'"]').css({
        top:base_marginTop*(r+1)+cellHeight*r,
        left:base_marginLeft*(c+1)+cellWidth*c
      })
    }
  }
}
