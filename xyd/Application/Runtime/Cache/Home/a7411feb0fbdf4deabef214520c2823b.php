...
<body>
	<div class="iframe-header">
		<ul class="breadcrumb">
			...
			<li><i class="fa fa-angle-right"></i>学生成就</li>
		</ul>
	</div>
	<div class="iframe-content p20">
		<div class="js_pageshow">
			<div class="search-row">
				<select class="select">
					...
					<option value="成就">成就</option>
				</select>
				<input type="text" name="search" class="search-field" placeholder="输入学生名字、成就">
			...
			<div class="operate-row">
				<div class="operate-left">
					...
					<button type="button" class="btn btn-blue btn-add"><i class="fa fa-plus"></i>新增学生</button>
				</div>
		...
		<table class="table-edit">
			<tr>
				...
				<td rowspan="5" class="tc">
					...
					<span class="edit-upload">
						<input type="file" id="edit_upload" name="upload_avatar"><span class="text">上传头像</span>
					</span>
			...
			<tr>
				<td class="tr">出生日期：</td>
				<td><input type="text" id="birth" class="edit-field js_disabled" value=""></td>
			</tr>
			<tr>
				<td class="tr">家庭住址：</td>
...
<script>
    ...
    function setEditTableData(data, isView) {
        ...
        UM.getEditor('myEditor').setContent((data.content == '' || data.content == null) ? '<p style="color: #999;">输入教师详情介绍。。。</p>' : data.content);
        if (isView === false) {
    ...
    (function(){
        ...
        $('.js_search').click(function(){
            ...
            $.ajax({
                type: 'POST',
                url: "<?php echo U('Admin/stu_search');?>",
        ...
        $('.btn-confirm').click(function(){
            var state = $(this).attr('data-state'),
                    url = '';
            if (state == 'edit') {  // 编辑
                url = "<?php echo U('Admin/stu_edit');?>";  // 编辑教师提交的url
            } else if (state == 'add') {  // 新增
                url = "<?php echo U('Admin/stu_add');?>";  // 新增教师提交的url
            }
